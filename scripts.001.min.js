...
var ab={};var x=ab.toString;var J=ab.hasOwnProperty;var C={};var ah="1.11.1",bH=function(e,i){return new bH.fn.init(e,i);},D=/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,bR=/^-ms-/,aV=/-([\da-z])/gi,N=function(e,i){return i.toUpperCase();
...
},isNumeric:function(e){return !bH.isArray(e)&&e-parseFloat(e)>=0;},isEmptyObject:function(i){var e;for(e in i){return false;}return true;},isPlainObject:function(b6){var i;
...
function ac(b5){var i=b5.length,e=bH.type(b5);if(e==="function"||bH.isWindow(b5)){return false;}if(b5.nodeType===1&&i){return true;}return e==="array"||i===0||typeof i==="number"&&i>0&&(i-1) in b5;
}var m=
/*!
...
}return 0;},dc=typeof undefined,cO=1<<31,cM=({}).hasOwnProperty,da=[],db=da.pop,cK=da.push,b6=da.push,cl=da.slice,cc=da.indexOf||function(dm){var dl=0,e=this.length;
for(;dl<e;dl++){if(this[dl]===dm){return dl;}}return -1;},b7="checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multiple|open|readonly|required|scoped",co="[\\x20\\t\\r\\n\\f]",b5="(?:\\\\.|[\\w-]|[^\\x00-\\xa0])+",cJ=b5.replace("w","w#"),c4="\\["+co+"*("+b5+")(?:"+co+"*([*^$|!~]?=)"+co+"*(?:'((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\"|("+cJ+"))|)"+co+"*\\]",cj=":("+b5+")(?:\\((('((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\")|((?:\\\\.|[^\\\\()[\\]]|"+c4+")*)|.*)\\)|)",cq=new RegExp("^"+co+"+|((?:^|[^\\\\])(?:\\\\.)*)"+co+"+$","g"),ct=new RegExp("^"+co+"*,"+co+"*"),cz=new RegExp("^"+co+"*([>+~]|"+co+")"+co+"*"),cs=new RegExp("="+co+"*([^\\]'\"]*?)"+co+"*\\]","g"),cQ=new RegExp(cj),cS=new RegExp("^"+cJ+"$"),c0={ID:new RegExp("^#("+b5+")"),CLASS:new RegExp("^\\.("+b5+")"),TAG:new RegExp("^("+b5.replace("w","w*")+")"),ATTR:new RegExp("^"+c4),PSEUDO:new RegExp("^"+cj),CHILD:new RegExp("^:(only|first|last|nth|nth-last)-(child|of-type)(?:\\("+co+"*(even|odd|(([+-]|)(\\d*)n|)"+co+"*(?:([+-]|)"+co+"*(\\d+)|))"+co+"*\\)|)","i"),bool:new RegExp("^(?:"+b7+")$","i"),needsContext:new RegExp("^"+co+"*[>+~]|:(even|odd|eq|gt|lt|nth|first|last)(?:\\("+co+"*((?:-\\d)?\\d*)"+co+"*\\)|)(?=[^-]|$)","i")},cb=/^(?:input|select|textarea|button)$/i,ck=/^h\d$/i,cN=/^[^{]+\{\s*\[native \w/,cP=/^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/,cZ=/[+~]/,cL=/'|\\/g,cr=new RegExp("\\\\([\\da-f]{1,6}"+co+"?|("+co+")|.)","ig"),c3=function(e,dm,i){var dl="0x"+dm-65536;
return dl!==dl||i?dm:dl<0?String.fromCharCode(dl+65536):String.fromCharCode(dl>>10|55296,dl&1023|56320);};try{b6.apply((da=cl.call(cH.childNodes)),cH.childNodes);
da[cH.childNodes.length].nodeType;}catch(cB){b6={apply:da.length?function(i,e){cK.apply(i,cl.call(e));}:function(dn,dm){var e=dn.length,dl=0;while((dn[e++]=dm[dl++])){}dn.length=e-1;
}};}function cu(dt,dl,dx,dz){var dy,dq,dr,dv,dw,dp,dn,e,dm,du;if((dl?dl.ownerDocument||dl:cH)!==cA){cX(dl);}dl=dl||cA;dx=dx||[];if(!dt||typeof dt!=="string"){return dx;
}if((dv=dl.nodeType)!==1&&dv!==9){return[];}if(c6&&!dz){if((dy=cP.exec(dt))){if((dr=dy[1])){if(dv===9){dq=dl.getElementById(dr);if(dq&&dq.parentNode){if(dq.id===dr){dx.push(dq);
return dx;}}else{return dx;}}else{if(dl.ownerDocument&&(dq=dl.ownerDocument.getElementById(dr))&&cD(dl,dq)&&dq.id===dr){dx.push(dq);return dx;}}}else{if(dy[2]){b6.apply(dx,dl.getElementsByTagName(dt));
return dx;}else{if((dr=dy[3])&&dg.getElementsByClassName&&dl.getElementsByClassName){b6.apply(dx,dl.getElementsByClassName(dr));return dx;}}}}if(dg.qsa&&(!c1||!c1.test(dt))){e=dn=c8;
dm=dl;du=dv===9&&dt;if(dv===1&&dl.nodeName.toLowerCase()!=="object"){dp=cg(dt);if((dn=dl.getAttribute("id"))){e=dn.replace(cL,"\\$&");}else{dl.setAttribute("id",e);
}e="[id='"+e+"'] ";dw=dp.length;while(dw--){dp[dw]=e+ch(dp[dw]);}dm=cZ.test(dt)&&cR(dl.parentNode)||dl;du=dp.join(",");}if(du){try{b6.apply(dx,dm.querySelectorAll(du));
return dx;}catch(ds){}finally{if(!dn){dl.removeAttribute("id");}}}}}return df(dt.replace(cq,"$1"),dl,dx,dz);}function cy(){var i=[];function e(dl,dm){if(i.push(dl+" ")>cm.cacheLength){delete e[i.shift()];
}return(e[dl+" "]=dm);}return e;}function ci(e){e[c8]=true;return e;}function ce(i){var dm=cA.createElement("div");try{return !!i(dm);}catch(dl){return false;
}finally{if(dm.parentNode){dm.parentNode.removeChild(dm);}dm=null;}}function di(dl,dn){var e=dl.split("|"),dm=dl.length;while(dm--){cm.attrHandle[e[dm]]=dn;
}}function b9(i,e){var dm=e&&i,dl=dm&&i.nodeType===1&&e.nodeType===1&&(~e.sourceIndex||cO)-(~i.sourceIndex||cO);if(dl){return dl;}if(dm){while((dm=dm.nextSibling)){if(dm===e){return -1;
}}}return i?1:-1;}function cv(e){return function(dl){var i=dl.nodeName.toLowerCase();return i==="input"&&dl.type===e;};}function ca(e){return function(dl){var i=dl.nodeName.toLowerCase();
return(i==="input"||i==="button")&&dl.type===e;};}function c5(e){return ci(function(i){i=+i;return ci(function(dl,dq){var dn,dm=e([],dl.length,i),dp=dm.length;
while(dp--){if(dl[(dn=dm[dp])]){dl[dn]=!(dq[dn]=dl[dn]);}}});});}function cR(e){return e&&typeof e.getElementsByTagName!==dc&&e;}dg=cu.support={};cI=cu.isXML=function(e){var i=e&&(e.ownerDocument||e).documentElement;
return i?i.nodeName!=="HTML":false;};cX=cu.setDocument=function(dl){var e,dm=dl?dl.ownerDocument||dl:cH,i=dm.defaultView;if(dm===cA||dm.nodeType!==9||!dm.documentElement){return cA;
}cA=dm;cn=dm.documentElement;c6=!cI(dm);if(i&&i!==i.top){if(i.addEventListener){i.addEventListener("unload",function(){cX();},false);}else{if(i.attachEvent){i.attachEvent("onunload",function(){cX();
});}}}dg.attributes=ce(function(dn){dn.className="i";return !dn.getAttribute("className");});dg.getElementsByTagName=ce(function(dn){dn.appendChild(dm.createComment(""));
return !dn.getElementsByTagName("*").length;});dg.getElementsByClassName=cN.test(dm.getElementsByClassName)&&ce(function(dn){dn.innerHTML="<div class='a'></div><div class='a i'></div>";
dn.firstChild.className="i";return dn.getElementsByClassName("i").length===2;});dg.getById=ce(function(dn){cn.appendChild(dn).id=c8;return !dm.getElementsByName||!dm.getElementsByName(c8).length;
});if(dg.getById){cm.find.ID=function(dq,dp){if(typeof dp.getElementById!==dc&&c6){var dn=dp.getElementById(dq);return dn&&dn.parentNode?[dn]:[];}};cm.filter.ID=function(dp){var dn=dp.replace(cr,c3);
return function(dq){return dq.getAttribute("id")===dn;};};}else{delete cm.find.ID;cm.filter.ID=function(dp){var dn=dp.replace(cr,c3);return function(dr){var dq=typeof dr.getAttributeNode!==dc&&dr.getAttributeNode("id");
return dq&&dq.value===dn;};};}cm.find.TAG=dg.getElementsByTagName?function(dn,dp){if(typeof dp.getElementsByTagName!==dc){return dp.getElementsByTagName(dn);
}}:function(dn,ds){var dt,dr=[],dq=0,dp=ds.getElementsByTagName(dn);if(dn==="*"){while((dt=dp[dq++])){if(dt.nodeType===1){dr.push(dt);}}return dr;}return dp;
};cm.find.CLASS=dg.getElementsByClassName&&function(dp,dn){if(typeof dn.getElementsByClassName!==dc&&c6){return dn.getElementsByClassName(dp);}};de=[];
c1=[];if((dg.qsa=cN.test(dm.querySelectorAll))){ce(function(dn){dn.innerHTML="<select msallowclip=''><option selected=''></option></select>";if(dn.querySelectorAll("[msallowclip^='']").length){c1.push("[*^$]="+co+"*(?:''|\"\")");
}if(!dn.querySelectorAll("[selected]").length){c1.push("\\["+co+"*(?:value|"+b7+")");}if(!dn.querySelectorAll(":checked").length){c1.push(":checked");}});
ce(function(dp){var dn=dm.createElement("input");dn.setAttribute("type","hidden");dp.appendChild(dn).setAttribute("name","D");if(dp.querySelectorAll("[name=d]").length){c1.push("name"+co+"*[*^$|!~]?=");
}if(!dp.querySelectorAll(":enabled").length){c1.push(":enabled",":disabled");}dp.querySelectorAll("*,:x");c1.push(",.*:");});}if((dg.matchesSelector=cN.test((cd=cn.matches||cn.webkitMatchesSelector||cn.mozMatchesSelector||cn.oMatchesSelector||cn.msMatchesSelector)))){ce(function(dn){dg.disconnectedMatch=cd.call(dn,"div");
cd.call(dn,"[s!='']:x");de.push("!=",cj);});}c1=c1.length&&new RegExp(c1.join("|"));de=de.length&&new RegExp(de.join("|"));e=cN.test(cn.compareDocumentPosition);
cD=e||cN.test(cn.contains)?function(dp,dn){var dr=dp.nodeType===9?dp.documentElement:dp,dq=dn&&dn.parentNode;return dp===dq||!!(dq&&dq.nodeType===1&&(dr.contains?dr.contains(dq):dp.compareDocumentPosition&&dp.compareDocumentPosition(dq)&16));
}:function(dp,dn){if(dn){while((dn=dn.parentNode)){if(dn===dp){return true;}}}return false;};cC=e?function(dp,dn){if(dp===dn){cV=true;return 0;}var dq=!dp.compareDocumentPosition-!dn.compareDocumentPosition;
if(dq){return dq;}dq=(dp.ownerDocument||dp)===(dn.ownerDocument||dn)?dp.compareDocumentPosition(dn):1;if(dq&1||(!dg.sortDetached&&dn.compareDocumentPosition(dp)===dq)){if(dp===dm||dp.ownerDocument===cH&&cD(cH,dp)){return -1;
}if(dn===dm||dn.ownerDocument===cH&&cD(cH,dn)){return 1;}return cG?(cc.call(cG,dp)-cc.call(cG,dn)):0;}return dq&4?-1:1;}:function(dp,dn){if(dp===dn){cV=true;
return 0;}var dv,ds=0,du=dp.parentNode,dr=dn.parentNode,dq=[dp],dt=[dn];if(!du||!dr){return dp===dm?-1:dn===dm?1:du?-1:dr?1:cG?(cc.call(cG,dp)-cc.call(cG,dn)):0;
}else{if(du===dr){return b9(dp,dn);}}dv=dp;while((dv=dv.parentNode)){dq.unshift(dv);}dv=dn;while((dv=dv.parentNode)){dt.unshift(dv);}while(dq[ds]===dt[ds]){ds++;
}return ds?b9(dq[ds],dt[ds]):dq[ds]===cH?-1:dt[ds]===cH?1:0;};return dm;};cu.matches=function(i,e){return cu(i,null,null,e);};cu.matchesSelector=function(dl,dn){if((dl.ownerDocument||dl)!==cA){cX(dl);
}dn=dn.replace(cs,"='$1']");if(dg.matchesSelector&&c6&&(!de||!de.test(dn))&&(!c1||!c1.test(dn))){try{var i=cd.call(dl,dn);if(i||dg.disconnectedMatch||dl.document&&dl.document.nodeType!==11){return i;
}}catch(dm){}}return cu(dn,cA,null,[dl]).length>0;};cu.contains=function(e,i){if((e.ownerDocument||e)!==cA){cX(e);}return cD(e,i);};cu.attr=function(dl,e){if((dl.ownerDocument||dl)!==cA){cX(dl);
}var i=cm.attrHandle[e.toLowerCase()],dm=i&&cM.call(cm.attrHandle,e.toLowerCase())?i(dl,e,!c6):undefined;return dm!==undefined?dm:dg.attributes||!c6?dl.getAttribute(e):(dm=dl.getAttributeNode(e))&&dm.specified?dm.value:null;
};cu.error=function(e){throw new Error("Syntax error, unrecognized expression: "+e);};cu.uniqueSort=function(dm){var dn,dp=[],e=0,dl=0;cV=!dg.detectDuplicates;
cG=!dg.sortStable&&dm.slice(0);dm.sort(cC);if(cV){while((dn=dm[dl++])){if(dn===dm[dl]){e=dp.push(dl);}}while(e--){dm.splice(dp[e],1);}}cG=null;return dm;
};cF=cu.getText=function(dp){var dn,dl="",dm=0,e=dp.nodeType;if(!e){while((dn=dp[dm++])){dl+=cF(dn);}}else{if(e===1||e===9||e===11){if(typeof dp.textContent==="string"){return dp.textContent;
}else{for(dp=dp.firstChild;dp;dp=dp.nextSibling){dl+=cF(dp);}}}else{if(e===3||e===4){return dp.nodeValue;}}}return dl;};cm=cu.selectors={cacheLength:50,createPseudo:ci,match:c0,attrHandle:{},find:{},relative:{">":{dir:"parentNode",first:true}," ":{dir:"parentNode"},"+":{dir:"previousSibling",first:true},"~":{dir:"previousSibling"}},preFilter:{ATTR:function(e){e[1]=e[1].replace(cr,c3);
e[3]=(e[3]||e[4]||e[5]||"").replace(cr,c3);if(e[2]==="~="){e[3]=" "+e[3]+" ";}return e.slice(0,4);},CHILD:function(e){e[1]=e[1].toLowerCase();if(e[1].slice(0,3)==="nth"){if(!e[3]){cu.error(e[0]);
}e[4]=+(e[4]?e[5]+(e[6]||1):2*(e[3]==="even"||e[3]==="odd"));e[5]=+((e[7]+e[8])||e[3]==="odd");}else{if(e[3]){cu.error(e[0]);}}return e;},PSEUDO:function(i){var e,dl=!i[6]&&i[2];
if(c0.CHILD.test(i[0])){return null;}if(i[3]){i[2]=i[4]||i[5]||"";}else{if(dl&&cQ.test(dl)&&(e=cg(dl,true))&&(e=dl.indexOf(")",dl.length-e)-dl.length)){i[0]=i[0].slice(0,e);
i[2]=dl.slice(0,e);}}return i.slice(0,3);}},filter:{TAG:function(i){var e=i.replace(cr,c3).toLowerCase();return i==="*"?function(){return true;}:function(dl){return dl.nodeName&&dl.nodeName.toLowerCase()===e;
};},CLASS:function(e){var i=b8[e+" "];return i||(i=new RegExp("(^|"+co+")"+e+"("+co+"|$)"))&&b8(e,function(dl){return i.test(typeof dl.className==="string"&&dl.className||typeof dl.getAttribute!==dc&&dl.getAttribute("class")||"");
});},ATTR:function(dl,i,e){return function(dn){var dm=cu.attr(dn,dl);if(dm==null){return i==="!=";}if(!i){return true;}dm+="";return i==="="?dm===e:i==="!="?dm!==e:i==="^="?e&&dm.indexOf(e)===0:i==="*="?e&&dm.indexOf(e)>-1:i==="$="?e&&dm.slice(-e.length)===e:i==="~="?(" "+dm+" ").indexOf(e)>-1:i==="|="?dm===e||dm.slice(0,e.length+1)===e+"-":false;
};},CHILD:function(i,dn,dm,dp,dl){var dr=i.slice(0,3)!=="nth",e=i.slice(-4)!=="last",dq=dn==="of-type";return dp===1&&dl===0?function(ds){return !!ds.parentNode;
...
while((dz=dz[dx])){if(dq?dz.nodeName.toLowerCase()===du:dz.nodeType===1){return false;}}dv=dx=i==="only"&&!dv&&"nextSibling";}return true;}dv=[e?dC.firstChild:dC.lastChild];
if(e&&dt){dE=dC[c8]||(dC[c8]={});ds=dE[i]||[];dA=ds[0]===dh&&ds[1];dD=ds[0]===dh&&ds[2];dz=dA&&dC.childNodes[dA];while((dz=++dA&&dz&&dz[dx]||(dD=dA=0)||dv.pop())){if(dz.nodeType===1&&++dD&&dz===dy){dE[i]=[dh,dA,dD];
break;}}}else{if(dt&&(ds=(dy[c8]||(dy[c8]={}))[i])&&ds[0]===dh){dD=ds[1];}else{while((dz=++dA&&dz&&dz[dx]||(dD=dA=0)||dv.pop())){if((dq?dz.nodeName.toLowerCase()===du:dz.nodeType===1)&&++dD){if(dt){(dz[c8]||(dz[c8]={}))[i]=[dh,dD];
}if(dz===dy){break;}}}}}dD-=dl;return dD===dp||(dD%dp===0&&dD/dp>=0);}};},PSEUDO:function(dm,dl){var e,i=cm.pseudos[dm]||cm.setFilters[dm.toLowerCase()]||cu.error("unsupported pseudo: "+dm);
if(i[c8]){return i(dl);}if(i.length>1){e=[dm,dm,"",dl];return cm.setFilters.hasOwnProperty(dm.toLowerCase())?ci(function(dq,ds){var dp,dn=i(dq,dl),dr=dn.length;
while(dr--){dp=cc.call(dq,dn[dr]);dq[dp]=!(ds[dp]=dn[dr]);}}):function(dn){return i(dn,0,e);};}return i;}},pseudos:{not:ci(function(e){var i=[],dl=[],dm=cU(e.replace(cq,"$1"));
return dm[c8]?ci(function(dp,du,ds,dq){var dt,dn=dm(dp,null,dq,[]),dr=dp.length;while(dr--){if((dt=dn[dr])){dp[dr]=!(du[dr]=dt);}}}):function(dq,dp,dn){i[0]=dq;
dm(i,null,dn,dl);return !dl.pop();};}),has:ci(function(e){return function(i){return cu(e,i).length>0;};}),contains:ci(function(e){return function(i){return(i.textContent||i.innerText||cF(i)).indexOf(e)>-1;
};}),lang:ci(function(e){if(!cS.test(e||"")){cu.error("unsupported lang: "+e);}e=e.replace(cr,c3).toLowerCase();return function(dl){var i;do{if((i=c6?dl.lang:dl.getAttribute("xml:lang")||dl.getAttribute("lang"))){i=i.toLowerCase();
return i===e||i.indexOf(e+"-")===0;}}while((dl=dl.parentNode)&&dl.nodeType===1);return false;};}),target:function(e){var i=dd.location&&dd.location.hash;
return i&&i.slice(1)===e.id;},root:function(e){return e===cn;},focus:function(e){return e===cA.activeElement&&(!cA.hasFocus||cA.hasFocus())&&!!(e.type||e.href||~e.tabIndex);
...
},first:c5(function(){return[0];}),last:c5(function(e,i){return[i-1];}),eq:c5(function(e,dl,i){return[i<0?i+dl:i];}),even:c5(function(e,dm){var dl=0;for(;
dl<dm;dl+=2){e.push(dl);}return e;}),odd:c5(function(e,dm){var dl=1;for(;dl<dm;dl+=2){e.push(dl);}return e;}),lt:c5(function(e,dn,dm){var dl=dm<0?dm+dn:dm;
for(;--dl>=0;){e.push(dl);}return e;}),gt:c5(function(e,dn,dm){var dl=dm<0?dm+dn:dm;for(;++dl<dn;){e.push(dl);}return e;})}};cm.pseudos.nth=cm.pseudos.eq;
for(cw in {radio:true,checkbox:true,file:true,password:true,image:true}){cm.pseudos[cw]=cv(cw);}for(cw in {submit:true,reset:true}){cm.pseudos[cw]=ca(cw);
}function cT(){}cT.prototype=cm.filters=cm.pseudos;cm.setFilters=new cT();cg=cu.tokenize=function(dn,dt){var i,dp,dr,ds,dq,dl,e,dm=c7[dn+" "];if(dm){return dt?0:dm.slice(0);
}dq=dn;dl=[];e=cm.preFilter;while(dq){if(!i||(dp=ct.exec(dq))){if(dp){dq=dq.slice(dp[0].length)||dq;}dl.push((dr=[]));}i=false;if((dp=cz.exec(dq))){i=dp.shift();
dr.push({value:i,type:dp[0].replace(cq," ")});dq=dq.slice(i.length);}for(ds in cm.filter){if((dp=c0[ds].exec(dq))&&(!e[ds]||(dp=e[ds](dp)))){i=dp.shift();
dr.push({value:i,type:ds,matches:dp});dq=dq.slice(i.length);}}if(!i){break;}}return dt?dq.length:dq?cu.error(dn):c7(dn,dl).slice(0);};function ch(dn){var dm=0,dl=dn.length,e="";
for(;dm<dl;dm++){e+=dn[dm].value;}return e;}function cp(dn,dl,dm){var e=dl.dir,dp=dm&&e==="parentNode",i=c2++;return dl.first?function(ds,dr,dq){while((ds=ds[e])){if(ds.nodeType===1||dp){return dn(ds,dr,dq);
}}}:function(du,ds,dr){var dv,dt,dq=[dh,i];if(dr){while((du=du[e])){if(du.nodeType===1||dp){if(dn(du,ds,dr)){return true;}}}}else{while((du=du[e])){if(du.nodeType===1||dp){dt=du[c8]||(du[c8]={});
if((dv=dt[e])&&dv[0]===dh&&dv[1]===i){return(dq[2]=dv[2]);}else{dt[e]=dq;if((dq[2]=dn(du,ds,dr))){return true;}}}}}};}function dj(e){return e.length>1?function(dp,dn,dl){var dm=e.length;
while(dm--){if(!e[dm](dp,dn,dl)){return false;}}return true;}:e[0];}function cx(dl,dp,dn){var dm=0,e=dp.length;for(;dm<e;dm++){cu(dl,dp[dm],dn);}return dn;
}function cY(e,dl,dm,dn,dr){var dp,du=[],dq=0,ds=e.length,dt=dl!=null;for(;dq<ds;dq++){if((dp=e[dq])){if(!dm||dm(dp,dn,dr)){du.push(dp);if(dt){dl.push(dq);
}}}}return du;}function cf(dl,i,dn,dm,dp,e){if(dm&&!dm[c8]){dm=cf(dm);}if(dp&&!dp[c8]){dp=cf(dp,e);}return ci(function(dA,dx,ds,dz){var dC,dy,du,dt=[],dB=[],dr=dx.length,dq=dA||cx(i||"*",ds.nodeType?[ds]:ds,[]),dv=dl&&(dA||!i)?cY(dq,dt,dl,ds,dz):dq,dw=dn?dp||(dA?dl:dr||dm)?[]:dx:dv;
if(dn){dn(dv,dw,ds,dz);}if(dm){dC=cY(dw,dB);dm(dC,[],ds,dz);dy=dC.length;while(dy--){if((du=dC[dy])){dw[dB[dy]]=!(dv[dB[dy]]=du);}}}if(dA){if(dp||dl){if(dp){dC=[];
dy=dw.length;while(dy--){if((du=dw[dy])){dC.push((dv[dy]=du));}}dp(null,(dw=[]),dC,dz);}dy=dw.length;while(dy--){if((du=dw[dy])&&(dC=dp?cc.call(dA,du):dt[dy])>-1){dA[dC]=!(dx[dC]=du);
}}}}else{dw=cY(dw===dx?dw.splice(dr,dw.length):dw);if(dp){dp(null,dx,dw,dz);}else{b6.apply(dx,dw);}}});}function c9(dr){var dl,dp,dm,dq=dr.length,du=cm.relative[dr[0].type],dv=du||cm.relative[" "],dn=du?1:0,ds=cp(function(i){return i===dl;
},dv,true),dt=cp(function(i){return cc.call(dl,i)>-1;},dv,true),e=[function(dx,dw,i){return(!du&&(i||dw!==dk))||((dl=dw).nodeType?ds(dx,dw,i):dt(dx,dw,i));
}];for(;dn<dq;dn++){if((dp=cm.relative[dr[dn].type])){e=[cp(dj(e),dp)];}else{dp=cm.filter[dr[dn].type].apply(null,dr[dn].matches);if(dp[c8]){dm=++dn;for(;
dm<dq;dm++){if(cm.relative[dr[dm].type]){break;}}return cf(dn>1&&dj(e),dn>1&&ch(dr.slice(0,dn-1).concat({value:dr[dn-2].type===" "?"*":""})).replace(cq,"$1"),dp,dn<dm&&c9(dr.slice(dn,dm)),dm<dq&&c9((dr=dr.slice(dm))),dm<dq&&ch(dr));
}e.push(dp);}}return dj(e);}function cW(dm,dl){var e=dl.length>0,dn=dm.length>0,i=function(dy,ds,dx,dw,dB){var dt,du,dz,dD=0,dv="0",dp=dy&&[],dE=[],dC=dk,dr=dy||dn&&cm.find.TAG("*",dB),dq=(dh+=dC==null?1:Math.random()||0.1),dA=dr.length;
if(dB){dk=ds!==cA&&ds;}for(;dv!==dA&&(dt=dr[dv])!=null;dv++){if(dn&&dt){du=0;while((dz=dm[du++])){if(dz(dt,ds,dx)){dw.push(dt);break;}}if(dB){dh=dq;}}if(e){if((dt=!dz&&dt)){dD--;
}if(dy){dp.push(dt);}}}dD+=dv;if(e&&dv!==dD){du=0;while((dz=dl[du++])){dz(dp,dE,ds,dx);}if(dy){if(dD>0){while(dv--){if(!(dp[dv]||dE[dv])){dE[dv]=db.call(dw);
}}}dE=cY(dE);}b6.apply(dw,dE);if(dB&&!dy&&dE.length>0&&(dD+dl.length)>1){cu.uniqueSort(dw);}}if(dB){dh=dq;dk=dC;}return dp;};return e?ci(i):i;}cU=cu.compile=function(e,dm){var dn,dl=[],dq=[],dp=cE[e+" "];
if(!dp){if(!dm){dm=cg(e);}dn=dm.length;while(dn--){dp=c9(dm[dn]);if(dp[c8]){dl.push(dp);}else{dq.push(dp);}}dp=cE(e,cW(dq,dl));dp.selector=e;}return dp;
};df=cu.select=function(dm,e,dn,dr){var dp,du,dl,dv,ds,dt=typeof dm==="function"&&dm,dq=!dr&&cg((dm=dt.selector||dm));dn=dn||[];if(dq.length===1){du=dq[0]=dq[0].slice(0);
...
if(!e){return dn;}else{if(dt){e=e.parentNode;}}dm=dm.slice(du.shift().value.length);}dp=c0.needsContext.test(dm)?0:du.length;while(dp--){dl=du[dp];if(cm.relative[(dv=dl.type)]){break;
}if((ds=cm.find[dv])){if((dr=ds(dl.matches[0].replace(cr,c3),cZ.test(du[0].type)&&cR(e.parentNode)||e))){du.splice(dp,1);dm=dr.length&&ch(du);if(!dm){b6.apply(dn,dr);
return dn;}break;}}}}(dt||cU(dm,dq))(dr,e,!c6,dn,cZ.test(dm)&&cR(e.parentNode)||e);return dn;};dg.sortStable=c8.split("").sort(cC).join("")===c8;dg.detectDuplicates=!!cV;
cX();dg.sortDetached=ce(function(e){return e.compareDocumentPosition(cA.createElement("div"))&1;});if(!ce(function(e){e.innerHTML="<a href='#'></a>";return e.firstChild.getAttribute("href")==="#";
})){di("type|href|height|width",function(i,e,dl){if(!dl){return i.getAttribute(e,e.toLowerCase()==="type"?1:2);}});}if(!dg.attributes||!ce(function(e){e.innerHTML="<input/>";
e.firstChild.setAttribute("value","");return e.firstChild.getAttribute("value")==="";})){di("value",function(i,e,dl){if(!dl&&i.nodeName.toLowerCase()==="input"){return i.defaultValue;
}});}if(!ce(function(e){return e.getAttribute("disabled")==null;})){di(b7,function(i,e,dm){var dl;if(!dm){return i[e]===true?e.toLowerCase():(dl=i.getAttributeNode(e))&&dl.specified?dl.value:null;
}});}return cu;})(a4);bH.find=m;bH.expr=m.selectors;bH.expr[":"]=bH.expr.pseudos;bH.unique=m.uniqueSort;bH.text=m.getText;bH.isXMLDoc=m.isXML;bH.contains=m.contains;
...
var bp,F,bn=/^(top|right|bottom|left)$/;if(a4.getComputedStyle){bp=function(e){return e.ownerDocument.defaultView.getComputedStyle(e,null);};F=function(ca,i,b9){var b7,b6,b8,e,b5=ca.style;
b9=b9||bp(ca);e=b9?b9.getPropertyValue(i)||b9[i]:undefined;if(b9){if(e===""&&!bH.contains(ca.ownerDocument,ca)){e=bH.style(ca,i);}if(X.test(e)&&aY.test(i)){b7=b5.width;
b6=b5.minWidth;b8=b5.maxWidth;b5.minWidth=b5.maxWidth=b5.width=e;e=b9.width;b5.width=b7;b5.minWidth=b6;b5.maxWidth=b8;}}return e===undefined?e:e+"";};}else{if(n.documentElement.currentStyle){bp=function(e){return e.currentStyle;
};F=function(b9,b6,b8){var ca,i,e,b5,b7=b9.style;b8=b8||bp(b9);b5=b8?b8[b6]:undefined;if(b5==null&&b7&&b7[b6]){b5=b7[b6];}if(X.test(b5)&&!bn.test(b6)){ca=b7.left;
i=b9.runtimeStyle;e=i&&i.left;if(e){i.left=b9.currentStyle.left;}b7.left=b6==="fontSize"?"1em":b5;b5=b7.pixelLeft+"px";b7.left=ca;if(e){i.left=e;}}return b5===undefined?b5:b5+""||"auto";
};}}function a6(e,i){return{get:function(){var b5=e();if(b5==null){return;}if(b5){delete this.get;return;}return(this.get=i).apply(this,arguments);}};}(function(){var ca,b8,b6,b9,b5,b7,i;
ca=n.createElement("div");ca.innerHTML="  <link/><table></table><a href='/a'>a</a><input type='checkbox'/>";b6=ca.getElementsByTagName("a")[0];b8=b6&&b6.style;
if(!b8){return;}b8.cssText="float:left;opacity:.5";C.opacity=b8.opacity==="0.5";C.cssFloat=!!b8.cssFloat;ca.style.backgroundClip="content-box";ca.cloneNode(true).style.backgroundClip="";
C.clearCloneStyle=ca.style.backgroundClip==="content-box";C.boxSizing=b8.boxSizing===""||b8.MozBoxSizing===""||b8.WebkitBoxSizing==="";bH.extend(C,{reliableHiddenOffsets:function(){if(b7==null){e();
}return b7;},boxSizingReliable:function(){if(b5==null){e();}return b5;},pixelPosition:function(){if(b9==null){e();}return b9;},reliableMarginRight:function(){if(i==null){e();
...
cd.style.marginRight=cd.style.width="0";ce.style.width="1px";i=!parseFloat((a4.getComputedStyle(cd,null)||{}).marginRight);}ce.innerHTML="<table><tr><td></td><td>t</td></tr></table>";
...
}p(v,cd,b6,cj);if(b8===2){return cj;}b5=cd.global;if(b5&&bH.active++===0){bH.event.trigger("ajaxStart");}cd.type=cd.type.toUpperCase();cd.hasContent=!q.test(cd.type);
...
}:bD;var az=0,ai={},ax=bH.ajaxSettings.xhr();if(a4.ActiveXObject){bH(a4).on("unload",function(){for(var e in ai){ai[e](undefined,true);}});}C.cors=!!ax&&("withCredentials" in ax);
...
i.html(e?bH("<div>").append(bH.parseHTML(cb)).find(e):cb);}).complete(ca&&function(cc,cb){i.each(ca,b5||[cc.responseText,cb,cc]);});}return this;};bH.expr.filters.animated=function(e){return bH.grep(bH.timers,function(i){return e===i.elem;
}).length;};var bW=a4.document.documentElement;function bq(e){return bH.isWindow(e)?e:e.nodeType===9?e.defaultView||e.parentWindow:false;}bH.offset={setOffset:function(b6,cg,ca){var cc,b9,e,b7,b5,ce,cf,cb=bH.css(b6,"position"),b8=bH(b6),cd={};
if(cb==="static"){b6.style.position="relative";}b5=b8.offset();e=bH.css(b6,"top");ce=bH.css(b6,"left");cf=(cb==="absolute"||cb==="fixed")&&bH.inArray("auto",[e,ce])>-1;
if(cf){cc=b8.position();b7=cc.top;b9=cc.left;}else{b7=parseFloat(e)||0;b9=parseFloat(ce)||0;}if(bH.isFunction(cg)){cg=cg.call(b6,ca,b5);}if(cg.top!=null){cd.top=(cg.top-b5.top)+b7;
}if(cg.left!=null){cd.left=(cg.left-b5.left)+b9;}if("using" in cg){cg.using.call(b6,cd);}else{b8.css(cd);}}};bH.fn.extend({offset:function(i){if(arguments.length){return i===undefined?this:this.each(function(b9){bH.offset.setOffset(this,i,b9);
...
}return bH;}));function pattern_checkerboard(){var d=40;var e=[];var b=Math.random()>0.5?true:false;var c=Math.random()>0.5?true:false;for(var f=0;f<d;
f++){e[f]=[];for(var a=0;a<d;a++){e[f][a]={};if(Math.random()<0.3||(b===c&&Math.random()>0.3)||(a===0&&f===0)){e[f][a].t=b?"w":"b";}else{e[f][a].t=false;
}e[f][a].r=Math.random();b=!b;}b=!b;}return e;}$(document).ready(function(){var s=50;var t=4;var p=0.013;var y={x:1,y:0.5};var w=1.5;var o={x:0,y:0};var N=true;
window.isFirefox=/firefox/i.test(navigator.userAgent);window.isSafari=!!navigator.userAgent.match(/Version\/[\d\.]+.*Safari/);window.isMobile=$("html").hasClass("mobile")||$("html").hasClass("tablet");
window.isIE=false;var L=$(window);var C=pattern_checkerboard();var K=TILES_DATA.size;var m=TILES_DATA.size+TILES_DATA.shadow_size;var r;var j={width:L.width(),height:L.height()};
var i;var E;var B=[];var k=[];var F=[];var f=[];var x=$("body");var I=window.isFirefox?$("html, body"):x;var H=$("#area");var c={x:null,y:null};var l=false;
var d=0;var J=false;var z=1;var v=false;var n=function(){if(window.isMobile){z=2;if($("html").hasClass("tablet")){z=1.5;}}i=new PIXI.Stage();E=PIXI.autoDetectRenderer(j.width*z,j.height*z,null,true);
E.view.id="pixi-canvas";document.body.appendChild(E.view);$("#pixi-canvas").css("transform","scale("+1/z+")");requestAnimFrame(a);};var M=function(){var Q,R;
for(var S=0;S<C.length;S++){for(var P=0;P<C[S].length;P++){Q=C[S][P];if(Q.t){R=TILES_DATA.dir[Q.t]+"/"+TILES_DATA.tiles[Q.t][Math.round(Q.r*(TILES_DATA.tiles[Q.t].length-1))].file;
Q.src=R;(function(V,U,T){u(V,U,T);})(R,P,S);}}}};var u=function(V,R,Q){var P,U,T,S;if(B.indexOf(V)>-1){U=e(R,Q);T=new PIXI.BaseTexture(P);S=new PIXI.Texture(T);
G(S,U);}else{if(V in k){U=e(R,Q);k[V].push(U);}else{k[V]=[];u(V,R,Q);P=new Image();P.onload=function(){l=true;r=this.width;T=new PIXI.BaseTexture(P);S=new PIXI.Texture(T);
B.push(V);for(var W=0;W<k[V].length;W++){G(S,k[V][W]);}};P.src=V;}}};var e=function(Q,P){var S=PIXI.Texture.fromImage("img/shadow.png");var R=new PIXI.Sprite(S);
R.position.x=K*Q-$(window).scrollLeft()+K/2;R.position.y=K*P-$(window).scrollTop()+K/2;R.pivot.x=K/2;R.pivot.y=K/2;R.shadow=true;R.ignore=true;R.alpha=0;
i.addChild(R);F.push(R);return R;};var G=function(P,R){var Q=new PIXI.Sprite(P);Q.connectedShadow=R;Q.position.x=R.position.x;Q.position.y=R.position.y;
Q.scale.x=1/r*K;Q.scale.y=1/r*K;Q.pivot.x=K/2;Q.pivot.y=K/2;Q.rotation=Math.PI/2*(-1+Math.ceil(Math.random()*4));Q.setInteractive(true);Q.ignore=true;h(Q);
i.addChild(Q);F.push(Q);q(Q);Q.ignore=false;Q.connectedShadow.ignore=false;};var h=function(P){P.mousedown=function(Q){P.flip(Q);};P.flip=function(Q){if(P.offsetAlpha<0.1){return;
}if(!window.isMobile){var Z=Math.floor(Q.originalEvent.pageX/K)*K;var Y=Math.floor(Q.originalEvent.pageY/K)*K;if(f.indexOf(Z+"-"+Y)!=-1){return;}f.push(Z+"-"+Y);
}var ab=new PIXI.Graphics();ab.beginFill(0);ab.drawRect(0,0,K,K);ab.pivot.x=K/2;ab.pivot.y=K/2;ab.scale.y=0;var X=9/1000;var aa;var S=0;var T=false;var U=false;
var R=1;var W=P.rotation/(Math.PI/2)/2==Math.round(P.rotation/(Math.PI/2)/2);(function V(ac){if(!U){requestAnimFrame(V);}var ad=ac-S;S=ac;if(!T&&R>0&&ad){R=Math.max(0,R-(ad*X));
if(!W){P.scale.x=R;}else{P.scale.y=R;}P.connectedShadow.scale.y=R;if(R===0){P.ignore=true;P.alpha=0;P.connectedShadow.scale.y=1/1000;ab.position.x=P.position.x;
ab.position.y=P.position.y;ab.isFlipped=true;ab.ignore=true;ab.connectedShadow=P.connectedShadow;i.addChild(ab);F.push(ab);T=true;}}else{if(T&&R<1){R=Math.min(1,R+(ad*X));
P.connectedShadow.scale.y=R;ab.scale.y=R;if(R==1){P.scale.x=1;P.scale.y=1;P.alpha=1;P.ignore=false;U=true;}}}})();};};var q=function(P){P.alpha=0;P.offsetAlpha=Math.random()*-4+0.5;
P.offsetAlpha=Math.min(0,P.offsetAlpha);if(!P.connectedShadow.ignore){P.connectedShadow.alpha=0;P.connectedShadow.offsetAlpha=P.offsetAlpha;}};var A=function(){var P=L.scrollTop();
var ad=L.scrollLeft();var Z=ad,ac=P;var X=$("#arrow-left"),aa=$("#arrow-up");var U=function(){var ah=false;if(L.width()+L.scrollLeft()>=H.width()-s){area.width+=t*L.width();
ah=true;}if(L.height()+L.scrollTop()>=H.height()-s){area.height+=t*L.height();ah=true;}if(ah){ae();}};var ae=function(){H.css({width:area.width+"px",height:area.height+"px"});
};var W=function(){var ai;for(var ah=0;ah<F.length;ah++){ai=F[ah];ai.position.x-=ad-Z;ai.position.y-=P-ac;if(ai.isFlipped){continue;}if(P>ac&&ai.position.y<-1*m){ai.position.y+=C.length*TILES_DATA.size;
}else{if(P<ac&&ai.position.y>z*(j.height+K)){ai.position.y-=C.length*TILES_DATA.size;}}if(ad>Z&&ai.position.x<-1*m){ai.position.x+=C.length*TILES_DATA.size;
}else{if(ad<Z&&ai.position.x>z*(j.width+K)){ai.position.x-=C.length*TILES_DATA.size;}}}Z=ad;ac=P;};var ag=function(){if(!l||window.isMobile){return;}if(d>1.5&&ad>0&&!X.is(":visible")){X.fadeIn();
}else{if(ad===0&&X.is(":visible")){X.fadeOut();}}if(d>1.5&&P>0&&!aa.is(":visible")){aa.fadeIn();}else{if(P===0&&aa.is(":visible")){aa.fadeOut();}}};W();
area.height=L.height()*t;area.width=L.width()*t;ae();g();ag();L.scroll(function(){P=L.scrollTop();ad=L.scrollLeft();U();W();ag();});if(window.isMobile){P=0;
ad=0;var T={x:0,y:0};var R={};var ab;var Y=false;var V=20;var S=1;var Q=0;var af;$("#pixi-canvas").get(0).addEventListener("touchstart",function(ai){N=false;
ab={x:0,y:0};Y=false;af=Q;var ah=ai.changedTouches[0];T.x=parseInt(ah.clientX,10);T.y=parseInt(ah.clientY,10);R.x=T.x;R.y=T.y;});$("#pixi-canvas").get(0).addEventListener("touchmove",function(ai){var ah=ai.changedTouches[0];
ad-=parseInt(ah.clientX,10)-T.x;P-=parseInt(ah.clientY,10)-T.y;ab.x=ab.x*0.7+(parseInt(ah.clientX,10)-T.x)*0.3;ab.y=ab.y*0.7+(parseInt(ah.clientY,10)-T.y)*0.3;
T.x=parseInt(ah.clientX,10);T.y=parseInt(ah.clientY,10);W();});$("#pixi-canvas").get(0).addEventListener("touchend",function(ai){if(T.x===R.x&&T.y==R.y){v={x:T.x*z,y:T.y*z};
return;}requestAnimFrame(ah);function ah(aj){if(!Y){Y=aj;Q=0;af=Math.max(Math.abs(ab.x),Math.abs(ab.y))*200;if(ab.x!==0){ab.x=Math.min(V,Math.abs(ab.x))*(Math.abs(ab.x)/ab.x);
}if(ab.y!==0){ab.y=Math.min(V,Math.abs(ab.y))*(Math.abs(ab.y)/ab.y);}requestAnimFrame(ah);return;}Q+=aj-Y;S=Math.min(1,Math.max(0,1-(Q/af)));ad-=ab.x*S;
P-=ab.y*S;W();if(S>0.01){requestAnimFrame(ah);}else{Y=false;}}});}L.bind("scroll mousedown DOMMouseScroll mousewheel keyup",function(ah){if(d>w&&(ah.which>0||ah.type==="mousedown"||ah.type==="mousewheel")){N=false;
}});};var g=function(){if(window.isIE){return;}var U={x:j.width*0.42,y:j.height*0.42};var W=0;var Z=2;var V=50;var T={};var P={x:0,y:0};var X;var S=null;
var R;L.mousemove(function(aa){T.x=L.width()-aa.clientX;T.y=L.height()-aa.clientY;});x.mouseleave(function(){T.x=-1;T.y=-1;});x.mouseenter(function(){X=setTimeout(Q,250);
});Q();function Q(aa){clearTimeout(X);requestAnimFrame(Q);if(S===null){S=aa;}R=aa-S;S=aa;if(isNaN(R)||T.x<0||T.y<0){return;}if(T.x<U.x){P.x=Y(T.x,U.x);
}else{if(T.x>L.width()-U.x){P.x=-1*Y(L.width()-T.x,U.x);}else{P.x=0;}}if(T.y<U.y){P.y=Y(T.y,U.y);}else{if(T.y>L.height()-U.y){P.y=-1*Y(L.height()-T.y,U.y);
}else{P.y=0;}}if(window.isSafari&&c.x!==null&&(I.scrollLeft()!==c.x||I.scrollTop()!==c.y)){c.x=I.scrollLeft();c.y=I.scrollTop();if(d>w&&(P.x!==0||P.y!==0)){N=false;
}return;}P.x=Math.round(P.x*R/16);P.y=Math.round(P.y*R/16);c.x=P.x+I.scrollLeft();c.y=P.y+I.scrollTop();if(P.x!==0||P.y!==0){if(d>w){N=false;}window.scrollTo(c.x,c.y);
}}function Y(ab,aa){return Math.max(W,(1-(ab/aa))*Z);}};var b=function(R,Q){var P=Math.floor(R/K);var S=Math.floor(Q/K);return C[S%C.length][P%C[0].length];
};var a=function(){requestAnimFrame(a);if(J){d+=(new Date().getTime()-J)/1000;}J=new Date().getTime();var Q=L.scrollLeft();for(var P=0;P<F.length;P++){tile=F[P];
if(tile.isFlipped&&tile.connectedShadow.position.x==tile.position.x&&tile.connectedShadow.position.y==tile.position.y){tile.connectedShadow.alpha=1;}if(tile.ignore){continue;
}if(tile.position.x<-m||tile.position.x>z*(j.width+K)||tile.position.y<-m||tile.position.y>z*(j.height+K)){if(!tile.shadow){q(tile);}}else{if(tile.alpha<1&&!tile.shadow){tile.offsetAlpha+=p;
tile.alpha=Math.min(1,Math.max(0,tile.offsetAlpha));if(!tile.connectedShadow.ignore){tile.connectedShadow.alpha=tile.alpha;}}}if(v&&!tile.shadow){if(v.x>=tile.position.x-K/2&&v.x-tile.position.x<K/2&&v.y>=tile.position.y-K/2&&v.y-tile.position.y<K/2){tile.flip({originalEvent:{pageX:v.x,pageY:v.y}});
v=false;}}}v=false;if(l&&N){o.x+=y.x;o.y+=y.y;window.scrollTo(o.x,o.y);}E.render(i);};var D=function(){$(window).on("resize orientationchange",function(){location.reload();
});};var O=function(){var R=window.navigator.userAgent;var Q=R.indexOf("MSIE ");var P=R.indexOf("Trident/");if(Q>0){return parseInt(R.substring(Q+5,R.indexOf(".",Q)),10);
}if(P>0){var S=R.indexOf("rv:");return parseInt(R.substring(S+3,R.indexOf(".",S)),10);}return false;};if(window.isSafari){x.addClass("is-safari");}else{if(O()){window.isIE=true;
N=false;}}n();setTimeout(function(){M();A();D();},50);});